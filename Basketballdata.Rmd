...
How each position attribute in to gaining points
```{r}
...
AST % vs eFG by college year. Should we consider Freshman rather than Senior? 
...
top_25s %>% filter(college_year == "Fr"|college_year =="So"|college_year =="Jr"|college_year =="Sr") %>% ggplot(aes(x=AST_per,y = eFG)) +
  geom_point(aes(color= college_year,alpha=0.5))+
  theme_linedraw() +
  xlab("Assistent Percentage") + 
