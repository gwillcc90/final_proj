...
Sport analytics can be applied in order to discovery "hidden" talent, rate players, judge players' performance, rank top teams, anticipate opponent behavior, , address sport cliches, maintain integrity in sport, so on and so forth. By looking at valuable and meaningful set of variables, we can gain some knowledge behind those numbers and trends. 

In this project, we work on the College Basketball created by Aditya Kumar from Kaggle, which contains past decade's NCAA college basketball data. Our goal is to look at meaningful continuous variables to evaluate players' performance and to see whether there are trends of these variables among categorical variables that we considered in this project which are player's role and player's academic status. For player's role, we categorized them into Post Players which included PowerForward Center, Center, Wing Forward, Stretch 4; and Guards which included Combo Guard, Scoring Point Guard, Wing Guard, Pure Point Guard. For player's academic status, there are freshman, sophomores, juniors, and seniors. 

Therefore, we hope this approach can help us to navigate which variables to look at to evaluate a good players in different role, as well as how the role of a player define their performance, and  whether academic status affects their performance to some extend.
...
First, we cleaned data to make it easier to work with by renaming some of the important variables to readable and easy to work on, also we decided to remove less-meaningful variables and variables with a great deal of data missing

We have two 3 main dataframe. First, we have a full data through 2010-2021, 2009 have too many data missing. Second, we have data frame that filter to past 3 seasons to look at newer data. Third, we have ranking top 25 team dataframe.
...
For who approach this dataset knowing little about Basketball, using correlation map would be a good way to form ideas of how is the relationship or correlation between all the varibles at very first steps of exploring data. Following that, we can research the meaning behind these correlations, after that we can visualize for a closer look into their meanings.
...
Finally, looking at assist/turnover ratio in necessary to see trends among the top 25 teams. Apparently, the average ast/tov ratio is one to one accounting for general turnovers that happen after shooting. The interesting part of this is that of the top 25 teams, the number of appearances in the most recent top 25 trends toward the higher your player's ast/tov ratio is the more likely you are to be in the top 25.
