...
Statistics are a good way to tell how you are doing in a sport and what you are best at. They can also help to determine where you and your team need to improve. We worked on the College Basketball created by Aditya Kumar from Kaggle, which contains past decade's NCAA college basketball data.

Our goal was to look at data trends based on role and academic status: freshman, sophmores, juniors, and seniors.
...
How's relationship among all the varibles?
...
Sports require teamwork! In evaluating a good team player, we measure player’s contributions and their impact on the court by BPM (Box Plus/Minus). The more time they contribute on the court, the more points they get. 
...
d<-son_bb_dat %>% slice_max(AST_per, n=50) %>% count(college_year) %>% filter(college_year == "Fr"|college_year =="So"|college_year =="Jr"|college_year =="Sr") %>% mutate(college_year = recode(college_year, "Fr"= "Freshman","So" = "Sorphomore", "Jr" = "Junior", "Sr" = "Senior")) %>% ggplot(aes(x=college_year,y = n,fill = college_year)) + geom_col()+ 
...
e<-son_bb_dat %>% slice_max(eFG, n=50) %>% count(college_year) %>% filter(college_year == "Fr"|college_year =="So"|college_year =="Jr"|college_year =="Sr")  %>% mutate(college_year = recode(college_year, "Fr"= "Freshman","So" = "Sorphomore", "Jr" = "Junior", "Sr" = "Senior")) %>% ggplot(aes(x=college_year,y = n,fill = college_year)) + geom_col()+ theme_linedraw() + 
  labs(fill = "College Year\n")+
  xlab("College Year") + 
...
Free Throws are important because they are uncontested shots 
...
new_bb %>%
  ...
  select(player, team, role, TP_per, year, yr)
```
...
at2 = some_new_bb %>%
  ...
  ggtitle(label = "Sum of Assist/Turnover Ratio of All Teams Appearing \nin the Top 25",
        subtitle = "Independent of Season Rank For Recent Seasons")+
  labs(color = "Appearances in Top 25",
       caption = "2018-2021")+
