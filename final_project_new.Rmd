...
#guard numeric data
top_fr_role_g_numdat = top_fr_role_g %>%
  select(-c(1:6,29,30,35,39,41:43,45,46))

#guard percentage data
top_fr_role_g_perdat = top_fr_role_g_numdat %>%
  select(contains("per")) %>%
  rename_with(~str_replace(., '_per',' %'))

#guard cont dat
top_fr_role_g_contdat = top_fr_role_g_perdat %>%
  select(-contains("per"))

#guard cat dat
top_fr_role_g_catdat = top_25_fr_2019[,1:6]
###



### powerforward/center data filters
...
#PfC num data
top_fr_role_PfC_numdat = top_fr_role_pfC %>%
  select(-c(1:6,29,30,35,39,41:43,45,46))

#PfC percentage dat
top_fr_role_PfC_perdat = top_fr_role_PfC_numdat %>%
  select(contains("per")) %>%
  rename_with(~str_replace(., '_per',' %'))

#PfC cont dat
top_fr_role_PfC_contdat = top_fr_role_PfC_perdat %>%
  select(-contains("per"))
...
#PfC cat dat
top_fr_role_PfC_catdat = top_25_fr_2019[,1:6]
###
